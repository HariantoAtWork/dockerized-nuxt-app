...
- HTTP server on `ADMIN_BIND`:`ADMIN_PORT` (defaults `0.0.0.0:9090`). Compose can publish `127.0.0.1:9090:9090` on the host.
...
- Status includes current/remote commit SHAs **and** subject messages (`currentCommitMessage`, `remoteCommitMessage`).
